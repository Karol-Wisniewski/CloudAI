...
class DeepLAuthor {
    ...
    final DeepL translator;

    DeepLAuthor(DeepL translator) {
        this.translator = translator;
    }

    String translate(String targetLang) {
        String input = AUTHOR.replace("<i>", "<i translate=\"no\">");
        String output = translator.translate(input, targetLang).replace("\\n", "\n");
        return new UnicodeUnescaper().translate(output);
    }
}
